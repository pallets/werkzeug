...
class Request(_SansIORequest):
    ...
    .. versionchanged:: 3.2
        The ``populate_request`` parameter is deprecated and will be removed in
        Werkzeug 4.0. ``environ["werkzeug.request"]`` will no longer be set.
    ...
    def __init__(
        ...
        if populate_request is not None:
            import warnings

            warnings.warn(
                "The 'populate_request' parameter is deprecated and will be"
                " removed in Werkzeug 4.0. 'environ[\"werkzeug.request\"]' will"
                " no longer be set.",
                DeprecationWarning,
                stacklevel=2,
            )
        else:
            populate_request = True  # type: ignore[assignment]
