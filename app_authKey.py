...
import numpy as np
from captchaModel.model import inference
from captchaModel.model import load_model
...
def api_predict():
    ...
    apiKeys = []
    with open("key_api.txt") as listApiKeys:
...
def home():
    ...
    return {"captcha": res, "time": f"{result_date:,.2f}s".replace("-", "")}
