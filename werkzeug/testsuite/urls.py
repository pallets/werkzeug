...
class URLsTestCase(WerkzeugTestCase):
    ...
    def test_quoting(self):
        self.assert_equal(urls.url_quote(u'\xf6\xe4\xfc'), '%C3%B6%C3%A4%C3%BC')
        self.assert_equal(urls.url_unquote(urls.url_quote(u'#%="\xf6')), u'#%="\xf6')
        self.assert_equal(urls.url_quote_plus('foo bar'), 'foo+bar')
        self.assert_equal(urls.url_unquote_plus('foo+bar'), 'foo bar')
        self.assert_equal(urls.url_encode({b'a': None, b'b': b'foo bar'}), u'b=foo+bar')
        self.assert_equal(urls.url_encode({u'a': None, u'b': u'foo bar'}), u'b=foo+bar')
        self.assert_equal(urls.url_fix(u'http://de.wikipedia.org/wiki/Elf (Begriffsklärung)'),
    ...
    def test_url_decoding(self):
        ...
        self.assert_equal(x[u'foo'], u'42')
        self.assert_equal(x[u'bar'], u'23')
        self.assert_equal(x[u'uni'], u'Hänsel')
        ...
        self.assert_equal(x[u'Üh'], u'Hänsel')
    ...
    def test_url_encoding(self):
        self.assert_equal(urls.url_encode({'foo': 'bar 45'}), u'foo=bar+45')
        d = {'foo': 1, 'bar': 23, 'blah': u'Hänsel'}
        self.assert_equal(urls.url_encode(d, sort=True), u'bar=23&blah=H%C3%A4nsel&foo=1')
        self.assert_equal(urls.url_encode(d, sort=True, separator=u';'), u'bar=23;blah=H%C3%A4nsel;foo=1')

    def test_sorted_url_encode(self):
        self.assert_equal(urls.url_encode({u"a": 42, u"b": 23, 1: 1, 2: 2}, sort=True), u'1=1&2=2&a=42&b=23')
        self.assert_equal(urls.url_encode({u'A': 1, u'a': 2, u'B': 3, 'b': 4}, sort=True,
    ...
    def test_iri_support(self):
        ...
        self.assert_equal(urls.uri_to_iri(b'http://xn--n3h.net/'),
                          u'http://\u2603.net/')
        self.assert_equal(
    ...
    def test_href(self):
        ...
        self.assert_equal(x(u'foo'), u'http://www.example.com/foo')
        self.assert_equal(x.foo(u'bar'), u'http://www.example.com/foo/bar')
        self.assert_equal(x.foo(u'bar', x=42), u'http://www.example.com/foo/bar?x=42')
        self.assert_equal(x.foo(u'bar', class_=42), u'http://www.example.com/foo/bar?class=42')
        self.assert_equal(x.foo(u'bar', {u'class': 42}), u'http://www.example.com/foo/bar?class=42')
        self.assert_raises(AttributeError, lambda: x.__blah__)

        x = urls.Href(u'blah')
        self.assert_equal(x.foo(u'bar'), u'blah/foo/bar')

        self.assert_raises(TypeError, x.foo, {u"foo": 23}, x=42)

        x = urls.Href(u'')
        self.assert_equal(x(u'foo'), u'foo')
    ...
    if 0:
        ...
        def test_href_past_root(self):
            ...
            self.assert_equal(base_href('../foo'), 'http://www.blagga.com/1/2/foo')
            self.assert_equal(base_href('../../foo'), 'http://www.blagga.com/1/foo')
            self.assert_equal(base_href('../../../foo'), 'http://www.blagga.com/foo')
            self.assert_equal(base_href('../../../../foo'), 'http://www.blagga.com/foo')
            self.assert_equal(base_href('../../../../../foo'), 'http://www.blagga.com/foo')
            self.assert_equal(base_href('../../../../../../foo'), 'http://www.blagga.com/foo')
    ...
    def test_url_unquote_plus_unicode(self):
        # was broken in 0.6
        self.assert_equal(urls.url_unquote_plus(u'\x6d'), u'\x6d')
